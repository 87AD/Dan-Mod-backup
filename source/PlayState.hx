...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'DanStage':
				{
					// curStage = 'DanStage';
					var bg:BGSprite = new BGSprite('Stage/DanStage', -890, -800, 0.9, 0.9);
					...
					var background:BGSprite = new BGSprite('Stage/DanBackground', -890, -670, 0.9, 0.9);
					background.setGraphicSize(Std.int(background.width * 1.45));
			...
			case 'danAngryStage':
				{
					// curStage = 'danAngryStage';
					var background2:BGSprite = new BGSprite('Stage/2phasebackground', 10, -250);
					// background2.setGraphicSize(Std.int(background2.width * 1.9));
					background2.updateHitbox();
					background2.scale.set(2.2, 2.2);
					background2.antialiasing = true;
					//	background2.scrollFactor.set(0.4, 0.4);
					// background2.active = false;
					add(background2);

					var bg2:BGSprite = new BGSprite('Stage/2phasestage', 20, 10);
					bg2.antialiasing = true;
					//	bg2.setGraphicSize(Std.int(bg2.width * 1.79));
					bg2.updateHitbox();
					bg2.scale.set(1.9, 1.9);
			...
			case 'danAxeStage':
				{
					// curStage = 'danAxeStage';
					var bg:BGSprite = new BGSprite('Stage/maybe3phase', 10, 10);
					...
					rgbEff.strength = 1;
					rgbEff.direction = 45;
					rgbEff.distance = 0.008;
					camHUD.setFilters([new ShaderFilter(rgbEff.shader)]);
				}
			case 'danFinalStage':
				{
					curStage = 'danFinalStage';

					var backgrounds:BGSprite = new BGSprite('Stage/4phasebackground', -890, -500, 0.9, 0.9);
					// backgrounds.setGraphicSize(Std.int(backgrounds.width * 1.2));
					...
					var bgs:BGSprite = new BGSprite('Stage/4phasestone', -890, -700, 0.9, 0.9);
					bgs.antialiasing = true;
					//	bgs.setGraphicSize(Std.int(bgs.width * 1.1));
					bgs.updateHitbox();
					bgs.scrollFactor.set(0.9, 0.9);
					bgs.active = false;
					add(bgs);

					var background3:BGSprite = new BGSprite('Stage/4phasestage', -890, -500, 0.9, 0.9);
					// background3.setGraphicSize(Std.int(background3.width * 1.2));
	...
	if (!foundFile)
	{
		...
		} if (foundFile)
		{
			...
			{
				remove(bg);
				if (Paths.formatToSongPath(SONG.song) == 'epic-song' /* && Paths.formatToSongPath(SONG.song) == 'emerald-green'*/)
				{
	...
	function axeAttack()
	{
		dad.flipX = true;
		dad.x += 720;
		dad.animation.play('singRIGHT-alt');
		boyfriend.animation.play('hurt');
		FlxG.camera.shake(0.06, 0.2);
