...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		var line:FlxSprite = new FlxSprite().loadGraphic(Paths.image('title/line'));
		line.scrollFactor.set();
		line.updateHitbox();
		line.antialiasing = true;
		add(line);

		dan = new FlxSprite(-117, 50);
		dan.scale.set(1.3, 1.3);
		dan.frames = Paths.getSparrowAtlas('title/Dans');
		dan.animation.addByPrefix('idle', 'Den', 24, false);
		dan.antialiasing = true;
		// dan.screenCenter(X);
		add(dan);

		logo = new FlxSprite(300, 1500);
		logo.scale.set(0.85, 0.85);
		logo.frames = Paths.getSparrowAtlas('title/Logo');
		logo.animation.addByPrefix('idle', 'logo idle', 24, false);
	...
	override function update(elapsed:Float)
	{
		...
		if (!transitioning && skippedIntro)
		{
			if (pressedEnter)
			{
				...
				FlxTween.tween(logo, {y: -1000}, 1.8, {ease: FlxEase.quadInOut});
				FlxTween.tween(dan, {y: -1000}, 1.8, {ease: FlxEase.quadInOut});
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			logo.visible = true;
			FlxTween.tween(logo, {y: -250}, 0.2, {ease: FlxEase.quadInOut});
